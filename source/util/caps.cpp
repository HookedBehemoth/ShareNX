...
std::string dateToString(const CapsAlbumFileDateTime &date) {
	...
	char dateString[30];
	snprintf(dateString, 30, temp, date.year, date.month, date.day, date.hour, date.minute, date.second);
...
std::string entryToFileName(const CapsAlbumEntry &entry) {
	...
	char fileName[33];
	snprintf(fileName, 33, temp, date.year, date.month, date.day, date.hour, date.minute, date.second, entry.file_id.content == CapsAlbumFileContents_ScreenShot ? "jpg" : "mp4");
